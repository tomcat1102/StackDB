...
// I don't really understand crc32c and it takes time to comprehend it .so I did a exact copy here
namespace stackdb {
namespace crc32c {
namespace {
const uint32_t BYTE_EXT_TBL[256] = {    // precomputed byte extension table
...
const uint32_t STRIDE_EXT_TBL_0[256] = {
...
const uint32_t STRIDE_EXT_TBL_1[256] = {
...
const uint32_t STRIDE_EXT_TBL_2[256] = {
...
const uint32_t STRIDE_EXT_TBL_3[256] = {
...
static constexpr const uint32_t CRC32_XOR = 0xffffffffU;

// reads a little-endian 32-bit integer from a 32-bit-aligned buffer.
inline uint32_t read_uint32_le(const uint8_t* buffer) {
    return decode_fixed_32(reinterpret_cast<const char*>(buffer));
}

// Returns the smallest address >= the given address that is aligned to N bytes.
// N must be a power of two.
template <int N>
constexpr inline const uint8_t* round_up(const uint8_t* pointer) {
...
// NEED: check cpu support for accelerating CRC32C calculation

uint32_t extend(uint32_t crc, const char* data, size_t n) {
  const uint8_t* p = reinterpret_cast<const uint8_t*>(data);
  const uint8_t* e = p + n;
  uint32_t l = crc ^ CRC32_XOR;

// process one byte at a time.
#define STEP1                              \
  do {                                     \
    int c = (l & 0xff) ^ *p++;             \
    l = BYTE_EXT_TBL[c] ^ (l >> 8); \
  } while (0)
// process one of the 4 strides of 4-byte data.
#define STEP4(s)                                                               \
  do {                                                                         \
    crc##s = read_uint32_le(p + s * 4) ^ STRIDE_EXT_TBL_3[crc##s & 0xff] ^ \
             STRIDE_EXT_TBL_2[(crc##s >> 8) & 0xff] ^                    \
             STRIDE_EXT_TBL_1[(crc##s >> 16) & 0xff] ^                   \
             STRIDE_EXT_TBL_0[crc##s >> 24];                             \
  } while (0)
...
#define STEP4W(w)                                   \
  ...
  if ((e - p) >= 16) {
    ...
    uint32_t crc0 = read_uint32_le(p + 0 * 4) ^ l;
    uint32_t crc1 = read_uint32_le(p + 1 * 4);
    uint32_t crc2 = read_uint32_le(p + 2 * 4);
    uint32_t crc3 = read_uint32_le(p + 3 * 4);
    p += 16;
